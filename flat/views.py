...
from .forms import FlatCreationForm, EnterFlatCreationForm, ChoreCreationForm, AnnouncementCreationForm
...
def new_chore(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            Chore(name=name, flat=flat).save()
            messages.success(request, f'Added new chore :c')
            return redirect('new-chore') # lita obowiazkow
    else:
